...
\textbf{Setup}: NQ dev, \textbf{267k passages}, granite-embedding-97m (384-dim).
\textbf{simdq} = asymmetric $b{=}2$ + fp32 rescore ($\alpha{=}10$) vs.\
\textbf{Milvus FLAT} = exact fp32 brute force. Identical embeddings \& chunking.

\vspace{6pt}
\begin{columns}
\column{0.48\textwidth}
\centering\small
\textbf{Quality --- statistically identical}\\[4pt]
\begin{tabular}{lcc}
\toprule
        & Milvus & simdq \\
        & fp32   & $b{=}2$ \\
\midrule
NDCG@10  & 0.576 & 0.576 \\
NDCG@100 & 0.610 & 0.609 \\
Match@10 & 0.874 & 0.872 \\
Match@100& 0.974 & 0.974 \\
MRR@10   & 0.525 & 0.524 \\
\bottomrule
\end{tabular}
...
\centering\small
\textbf{Speed --- retrieval kernel}\\[4pt]
\begin{tabular}{lcc}
\toprule
                   & Milvus & simdq \\
\midrule
Median lat./query  & 7.6 ms  & \textbf{\textcolor{key}{3.2 ms}} \\
Kernel throughput  & 131 q/s & \textbf{\textcolor{key}{302 q/s}} \\
Scan-tier bytes/doc& 1536    & \textbf{\textcolor{key}{96}} \\
\bottomrule
\end{tabular}\\[6pt]
{\scriptsize fp16 rescore tier touched only for top-$K'$ candidates.}
\end{columns}

\vspace{8pt}
\onslide<2->{
\textcolor{hi}{\textbf{Same NDCG to $\pm 0.001$, $\approx 2.3\times$ faster scan, $16\times$ smaller scan codes.}}
Asymmetric $b{=}2$ + rescore reproduces \emph{exact} fp32 ranking on a real corpus ---
the $50$M-scale kernel speed is not bought with quality.\\[4pt]
\onslide<3->{
\small With \textbf{batched query encoding}, end-to-end search throughput widens further:
\textbf{286 q/s} (simdq) vs.\ \textbf{66 q/s} (Milvus, per-query encode).
}
}
\end{frame}
