...
\textbf{Same code, two CPUs}, identical harness, ms/query at 16-way concurrent.
Quality identical (lossless rescore).

\vspace{3pt}
\begin{center}\small
\begin{tabular}{l c cc c}
\toprule
Mode (dim)                  & B/doc & 9950X3D     & 5955WX     & ratio \\
                            &       & (AVX-512)   & (AVX2)     &       \\
\midrule
Milvus FLAT (fp32), 384d    & 1536  & 5.0  & 5.10  & 1.0$\times$ \\
Milvus FLAT (fp32), 768d    & 3072  & 11.7 & 10.90 & 0.9$\times$ \\
asym $b{=}2$, 384d          & 96    & 0.43 & 3.76  & 8.7$\times$ \\
asym $b{=}2$, 768d          & 192   & 0.81 & 7.23  & 8.9$\times$ \\
\textbf{1-bit (Hamming)}, 384d & \textbf{48} & 0.23 & \textbf{\textcolor{key}{0.11}} & \textbf{0.5$\times$} \\
\textbf{1-bit (Hamming)}, 768d & \textbf{96} & 0.40 & \textbf{\textcolor{key}{0.17}} & \textbf{0.4$\times$} \\
\bottomrule
\end{tabular}
\end{center}
...
\onslide<2->{
\small
\textbf{Milvus FLAT is ISA-blind} — it's memory + RPC bound. The 5955WX's
8-channel DDR4 narrowly beats the 9950X3D's 2-channel DDR5 once the corpus
exits cache.
}
...
\onslide<3->{
\small
\textbf{Asym $b{=}2$ collapses on AVX2.} Lever-1 unpack is \texttt{vpmultishiftqb}
(AVX-512 VBMI only); without it, scalar unpack stalls the FMA pipeline. Single-thread
768d: \textbf{13~ms} (AVX-512+Lever-1) vs \textbf{134~ms} (AVX2) — $\sim$10$\times$.
}

\vspace{1pt}
\onslide<4->{
\small
\textbf{1-bit goes \emph{faster} on AVX2.} The kernel is \texttt{popcnt}+\texttt{xor},
already memory-bound; the Threadripper's wider DDR4 bus + larger L3 bandwidth wins.
}

\vspace{1pt}
\onslide<5->{
\textcolor{hi}{\textbf{Takeaway:}} Asym vs.\ 1-bit ratio: $\sim$2$\times$ on
AVX-512, \textbf{$\sim$42$\times$ on AVX2}.
\textcolor{acc}{The deployment default flips with the ISA} — an AVX2
\texttt{vpshufb}-LUT analog of Lever-1 would close most of the asym gap.
}
\end{frame}
