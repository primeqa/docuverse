...
\bottomrule
\end{tabular}\\[4pt]
{\scriptsize all with fp32 rescore ($\alpha{=}10$). Match@100 within 0.6\% of fp32.}
...
\toprule
            ...
            & fp32   &        &       \\
\midrule
97m  \scriptsize(384d)  & 5.0 & 0.43 & \textbf{\textcolor{key}{0.23}} \\
311m \scriptsize(768d)  & 11.7 & 0.81 & \textbf{\textcolor{key}{0.40}} \\
\bottomrule
\end{tabular}\\[5pt]
{\scriptsize search-only, 16-way concurrent, same 276k vectors / machine.\\
bytes/doc: 1-bit 48/96, $b{=}2$ 96/192, fp32 1536/3072.}
\end{columns}

\vspace{6pt}
\onslide<2->{
\textcolor{hi}{\textbf{Quality holds for all modes} --- the bit-hashing science works.}
\textbf{1-bit Hamming + fp32 rescore is now the fastest \emph{and} smallest} ---
up to \textbf{$\sim$29$\times$ faster than Milvus FLAT}, lossless at 768d:
}
\onslide<3->{
\begin{itemize}\setlength\itemsep{1pt}\small
  \item Identity projection \texttt{W@q} let OpenBLAS oversubscribe the scan
    threads --- skip it ($49\to3.9$~ms); then an in-register
    \texttt{vpmultishiftqb}+\texttt{vpshufb} unpack replaced the $b{=}2$ scalar
    code-unpack ($\sim$4$\times$, $\to0.9$~ms, bit-identical).
  \item \textbf{1-bit:} storing codes \textbf{SoA} killed a per-query AoS$\to$SoA
    transpose ($3.4\to0.4$~ms, $8.6\times$); the 1-bit scan now stays L3-resident.
    fp32 rescore recovers exact ranking --- lossless at 768d, NDCG@10 $-0.001$ at 384d.
\end{itemize}
}
\end{frame}
