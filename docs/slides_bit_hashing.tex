...
% ---------------------------------------------------------------
\begin{frame}{The encoder is its own random projection}

SimHash requires the projection to be isotropic Gaussian.
Trained encoder dimensions are \emph{not} random — but empirically, skipping $W$ works.\\[8pt]

\onslide<2->{
\begin{columns}
\column{0.48\textwidth}
  \textbf{Classic LSH recipe}:
  \begin{enumerate}
    \item Embed to $\mathbb{R}^D$
    \item Multiply by random $W \in \mathbb{R}^{D \times D}$
    \item Take signs
  \end{enumerate}
  Cost: $O(D^2)$ projection per vector.
...
\column{0.48\textwidth}
  ...
  \begin{enumerate}
    \item Embed to $\mathbb{R}^D$
    \item Take signs directly
  \end{enumerate}
  Contrastive training encourages near-isotropy across dimensions.\\[4pt]
  \textcolor{hi}{Not proven — but competitive BEIR recall without $W$ is the evidence.}
  }
\end{columns}
}
...
\vspace{10pt}
\onslide<4->{
\textcolor{acc}{\textbf{Consequence: the 1-bit code costs nothing extra at ingest time}}\\
— just call \texttt{sign()} on the output layer.
}
\end{frame}
