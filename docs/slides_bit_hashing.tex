...
unrelated pairs. \textbf{$\sigma_H = 10$ is small.}\\[8pt]
The gap between a true match ($\cos\theta = 0.9$, $E[H]\approx110$) and a near-miss
($\cos\theta = 0.7$, $E[H]\approx194$)
corresponds to $\approx 84$ bits in Hamming space — well above the noise.\\[8pt]
}
\onslide<3->{
...
SimHash assumes the projection is \textbf{isotropic Gaussian}. A trained encoder is
\emph{not} random --- so does skipping $W$ really preserve cosine? \textbf{We measured it.}\\[4pt]
...
\vspace{2pt}
\onslide<3->{
Real embeddings are \textbf{strongly anisotropic} with \emph{rogue} dimensions ---
naked $\operatorname{sign}(x)$ wastes bits. Two fixes:
\begin{itemize}\setlength\itemsep{1pt}
  \item \textbf{Cheap (affine):} center + per-dim standardize. Fixes sign balance
    ($0.50\!\to\!0.03$) and the mean offset. \textcolor{key}{Universal win} --- but does
    \emph{not} decorrelate (effective rank stays $\approx 0.21$).
  \item \textbf{Structural:} a rotation $W$ to decorrelate the axes --- but \emph{which} $W$? (next)
\end{itemize}
}
\end{frame}
...
\textbf{Symmetric}: quantize \emph{both} query and database to $b$ bits.\\[4pt]
Per-dimension error: $(q_w{+}\varepsilon_q)(y_w{+}\varepsilon_y) - q_w y_w
= q_w\varepsilon_y + y_w\varepsilon_q + \varepsilon_q\varepsilon_y$\\[2pt]
...
\vspace{4pt}
\onslide<3->{
\textbf{The $b=1$ case is more severe.}
Quantizing a unit-norm vector to $\pm 1$ per dim inflates its norm from $1$ to $\sqrt{D}$,
collapsing all magnitude information.
Asymmetric avoids this on the query side entirely.
}
\end{frame}
...
\vspace{10pt}
\onslide<2->{
Each vector gets a \textbf{per-vector scale} $s_i = \|y_i\|/\sqrt{d}$, stored as fp16.\\[4pt]
After dividing by $s_i$, the distribution has unit MSE, so the levels straddle the
empirical quartiles with no wasted range.\\[8pt]
...
\textbf{Setup}: NQ dev ($\sim$270k passages). \textbf{simdq} = asymmetric $b{=}2$ +
fp32 rescore ($\alpha{=}10$) vs.\ \textbf{Milvus FLAT} = exact fp32 brute force.
Two encoders, identical embeddings \& chunking each.

\vspace{4pt}
\begin{columns}
\column{0.54\textwidth}
\centering\small
\textbf{Quality --- lossless on both}\\[3pt]
\begin{tabular}{l cc cc}
\toprule
 & \multicolumn{2}{c}{NDCG@10} & \multicolumn{2}{c}{Match@100} \\
 & fp32 & simdq & fp32 & simdq \\
\midrule
granite-97m \scriptsize(384d)  & 0.576 & 0.576 & 0.974 & 0.974 \\
granite-311m \scriptsize(768d) & 0.607 & 0.607 & 0.983 & 0.984 \\
\bottomrule
\end{tabular}

\column{0.46\textwidth}
\centering\small
\textbf{Search speed (ms/query)}\\[3pt]
\begin{tabular}{l cc c}
\toprule
            & Milvus & simdq & \\
            & fp32   & $b{=}2$ & $\times$ \\
\midrule
97m  \scriptsize(384d)  & 6.3 & \textbf{\textcolor{key}{0.42}} & 15 \\
311m \scriptsize(768d)  & 12.2 & \textbf{\textcolor{key}{0.89}} & 14 \\
\bottomrule
\end{tabular}\\[5pt]
{\scriptsize search-only, 16-way concurrent, same 276k vectors / machine.\\
codes $16\times$ smaller (96 vs 1536 B/doc).}
\end{columns}

\vspace{6pt}
\onslide<2->{
\textcolor{hi}{\textbf{Quality is lossless on both ($\pm0.001$ NDCG)} --- the bit-hashing
science holds.} simdq is \textbf{$\sim$14--15$\times$ faster than Milvus FLAT} at both dims:
}
\onslide<3->{
\begin{itemize}\setlength\itemsep{1pt}\small
  \item Two fixes got 768-dim there. \textbf{(1)} the per-query identity projection
    \texttt{W@q} let OpenBLAS spawn a thread pool that oversubscribed the scan
    threads --- skip the identity matmul ($49\to3.9$~ms).
  \item \textbf{(2)} the AVX-512 scan unpacked 2-bit codes with a scalar
    loop + stack roundtrip; an in-register \texttt{vpmultishiftqb}+\texttt{vpshufb}
    unpack is $\sim$4$\times$ faster ($\to0.9$~ms), bit-identical. Codes stay $16\times$ smaller.
\end{itemize}
}
\end{frame}
