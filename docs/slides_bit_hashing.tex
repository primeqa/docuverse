...
\bottomrule
\end{tabular}\\[3pt]
{\scriptsize B/doc: 1-bit 48/96, $b{=}2$ 96/192, fp32 1536/3072.}
\end{columns}

\vspace{3pt}
\onslide<2->{
\textcolor{hi}{\textbf{Quality holds for all modes.}} \textbf{1-bit Hamming + fp32
rescore is fastest \emph{and} smallest} --- up to $\sim$29$\times$ vs Milvus FLAT, lossless at 768d.
}
\onslide<3->{
\begin{itemize}\setlength\itemsep{1pt}\footnotesize
  \item $b{=}2$ 768d $49\to0.9$~ms: skip identity \texttt{W@q} (BLAS oversubscription)
    {}+ in-register \texttt{vpmultishiftqb} code-unpack.
  \item \textbf{1-bit}: SoA storage kills the per-query AoS$\to$SoA transpose
    ($3.4\to0.4$~ms, L3-resident); fp32 rescore keeps it lossless.
\end{itemize}
}
\end{frame}
