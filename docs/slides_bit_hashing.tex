...
311m \scriptsize(768d)  & 9.4 & \textcolor{acc}{49} \\
\bottomrule
\end{tabular}\\[5pt]
...
science holds.} The asymmetric $b{=}2$ scan is \emph{compute}-bound, so:
}
\onslide<3->{
\begin{itemize}\setlength\itemsep{1pt}\small
  \item at \textbf{384-dim} the unpack beats optimized fp32 FLAT ($2.3\times$ faster);
  \item at \textbf{768-dim} it \textcolor{acc}{loses} ($\approx$5$\times$ slower) --- a kernel-efficiency gap,
    not a memory one. Codes are still $16\times$ smaller; the $768$-dim unpack kernel is the next lever.
\end{itemize}
}
\end{frame}
