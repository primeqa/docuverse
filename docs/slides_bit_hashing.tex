...
\centering\small
\textbf{Scan speed (ms/query)}\\[3pt]
\begin{tabular}{l cc}
\toprule
            & Milvus & simdq \\
            & fp32   & $b{=}2$ \\
\midrule
97m  \scriptsize(384d)  & 7.6 & \textbf{\textcolor{key}{3.2}} \\
311m \scriptsize(768d)  & 9.4 & \textbf{\textcolor{key}{3.9}} \\
\bottomrule
\end{tabular}\\[5pt]
{\scriptsize scan codes $16\times$ smaller (96 vs 1536 B/doc) either way.}
\end{columns}

\vspace{6pt}
\onslide<2->{
\textcolor{hi}{\textbf{Quality is lossless on both ($\pm0.001$ NDCG)} --- the bit-hashing
science holds.} simdq \textbf{beats Milvus FLAT at both dims}:
}
\onslide<3->{
\begin{itemize}\setlength\itemsep{1pt}\small
  \item \textbf{384-dim}: $2.3\times$ faster; \textbf{768-dim}: $2.4\times$ faster.
  \item An earlier $768$-dim ``regression'' ($49$~ms) was \textbf{not} the kernel:
    the per-query identity projection \texttt{W@q} let OpenBLAS spawn its own
    thread pool, which oversubscribed the cores against the scan threads. Skipping
    the identity matmul fixed it ($49 \to 3.9$~ms). Codes stay $16\times$ smaller.
\end{itemize}
}
\end{frame}
