...
class BiEncoderReranker(Reranker):
    ...
    def _rerank(self, answer_list, show_progress):
        ...
        for answer in answer_list:
            # Embed every retrieved doc (not just top_k): similarities below are
            # computed over the docs we rerank, and _build_sorted_list re-appends
            # the tail beyond top_k unreranked.
        ...
        for qid, answer in tqdm(enumerate(answer_list), desc="Computing Cosine: ",
            ...
            reranked_docs = answer.top_k(self.top_k)
            similarity_scores = self.similarity(
                qembed, [embeddings[id2pos[doc.id]] for doc in reranked_docs])
            ...
            if self.config.reranker_combination_type == 'weight':
                ...
                hybrid_similarities = [
                    similarity * weight + result.score * (1 - weight)
                    for result, similarity in zip(reranked_docs, similarity_scores)
                ]
                ...
                k = num_reranked
                hybrid_similarities = [
                    1.0 / (k + i + 1) + 1.0 / (k + idx_to_rerank_idx[i] + 1)
                    for i in range(num_reranked)
                ]
            else:  # 'none'
                hybrid_similarities = similarity_scores
            reranked_list = self._build_sorted_list(answer, hybrid_similarities)
