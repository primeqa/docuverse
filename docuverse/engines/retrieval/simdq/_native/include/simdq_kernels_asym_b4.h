 ...
 */
static inline void scan_asym_b4_d768_shard_topk(const uint8_t *codes, size_t N,
                                                size_t i0, size_t i1,
                                                const float *q, int K,
...
static inline void scan_asym_b4_d768_topk(const uint8_t *codes, size_t N,
                                          const float *q, int K,
                                          float *out_s, int64_t *out_i) {
    scan_asym_b4_d768_shard_topk(codes, N, 0, N, q, K, out_s, out_i);
}
 ...
 * scan_asym_b4_d768_shard_topk on its range, then a critical-section
 ...
 */
static inline void scan_asym_b4_d768_topk_parallel(const uint8_t *codes, size_t N,
                                                   const float *q, int K,
                                                   float *gs, int64_t *gi) {
    assert(K > 0 && K <= 256);
    ...
    #pragma omp parallel
    {
        ...
        if (i0 < i1) {
            ...
            scan_asym_b4_d768_shard_topk(codes, N, i0, i1, q, K, ls, li);
            #pragma omp critical
            for (int r = 0; r < K; r++) {
                if (li[r] < 0) continue;
